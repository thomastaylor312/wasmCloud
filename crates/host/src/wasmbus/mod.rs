...
impl Queue {
    ...
    async fn new(
        ...
        // NOTE(thomastaylor312): I tried to use a join all, but the futures were 2 different impl
        // types and I was having issues type erasing them
        let (registries, pings, auctions, links, claims, actors, providers, labels, hosts, config) =
            try_join!(
                nats.subscribe(format!("{topic_prefix}.{lattice}.registry.put",)),
                nats.subscribe(format!("{topic_prefix}.{lattice}.host.ping",)),
                ...
                nats.subscribe(format!("{topic_prefix}.{lattice}.actor.*.{host_id}")),
                nats.subscribe(format!("{topic_prefix}.{lattice}.provider.*.{host_id}")),
                nats.subscribe(format!("{topic_prefix}.{lattice}.label.*.{host_id}")),
                nats.subscribe(format!("{topic_prefix}.{lattice}.host.*.{host_id}")),
                nats.queue_subscribe(
        ...
        Ok(Self {
            all_streams: futures::stream::select_all([
                registries, pings, auctions, links, claims, actors, providers, labels, hosts,
                config,
            ]),
        })
    }
